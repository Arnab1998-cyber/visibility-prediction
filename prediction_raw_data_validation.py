...
class raw_data_validation:
    ...
    def move_bad_files_to_archive(self):
        ...
        try:
            source="Prediction_raw_checking_file/bad_csv_files/"
            if os.path.isdir(source):
                path='Prediction_archive_bad_data'
                if not os.path.isdir(path):
                    os.makedirs(path)
                dest='Prediction_archive_bad_data/bad_data_'+str(date)+' :: '+str(time)
    ...
    def validation_name_raw(self, regex, length_of_date,length_of_time):
        self.log.apply_log("Logs/log.txt", msg='name pattern validation started')
    ...
    def check_null_column(self):
        try:
            self.log.apply_log("Logs/log.txt", msg='null column validation started')
            for f in os.listdir(self.good_data_file):
                df=pd.read_csv(self.good_data_file+f)
                for i in df:
                    if df[i].count()==0:
