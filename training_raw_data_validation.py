...
class raw_data_validation:
    ...
    def values_from_schema(self):
        try:
            ...
            if not os.path.isdir('Logs/'):
                os.makedirs('Logs/')
            self.log.apply_log("Logs/log.txt", msg=msg)
    ...
    def manual_regex_creation(self):
        try:
            ...
            self.log.apply_log("Training_Logs/raw_data_check.txt",msg=msg)
            return regex
        except Exception as e:
               raise e
    ...
    def move_bad_files_to_archive(self):
        ...
        try:
            ...
            if os.path.isdir(source):
                ...
                self.log.apply_log("Logs/log.txt",msg="bad csv files moved to archive")
                if os.path.isdir(source):
                    os.removedirs(source)
                self.log.apply_log("Logs/log.txt",msg='bad csv files folder removed')
        except Exception as e:
            raise e


    def validation_name_raw(self, regex, length_of_date,length_of_time):
        self.log.apply_log("Training_Logs/raw_data_check.txt", msg='name pattern validation started')
                ...
                if len(l1)==3 and len(l1[0])==10:
                    if len(l1[1])==length_of_date:
                        if len(l1[2])==length_of_time:
                            ...
                            self.log.apply_log("Logs/log.txt",msg='{} copied into good folder'.format(file))
                        else:
                             shutil.copy('Training_Batch_Files/' + file, 'Training_raw_checking_file/bad_csv_files')
                             self.log.apply_log("Logs/log.txt",msg='{} copied into bad folder'.format(file))
                    else:
                        shutil.copy('Training_Batch_Files/' + file, 'Training_raw_checking_file/bad_csv_files')
                        self.log.apply_log("Logs/log.txt",msg='{} copied into bad folder'.format(file))
                else:
    ...
    def validation_col_length_raw(self, col_length):
        try:
            ...
            self.log.apply_log("Logs/log.txt", msg='column length validation started')
            for f in os.listdir(self.good_data_path):
                ...
                if df.shape[1]==col_length:
                    ...
                    self.log.apply_log("Logs/log.txt",msg='{} copied to bad folder'.format(f))
        except Exception as e:
            raise e

    def check_null_column(self):
        try:
            self.log.apply_log("Training_Logs/raw_data_check.txt", msg='null column validation started')
            for f in os.listdir(self.good_data_path):
                df=pd.read_csv(self.good_data_path+'/'+f)
                for i in df:
                    if df[i].count()==0:
    ...
    def data_type_validation(self):
         try:
             self.log.apply_log("Logs/log.txt", msg='data type validation started')
             for f in os.listdir(self.good_data_path):
                 df=pd.read_csv(self.good_data_path+'/'+f)
